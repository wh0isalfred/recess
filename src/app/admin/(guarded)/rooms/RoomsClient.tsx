...
function RoomDrawer({ state, onClose, onSaved }: { state: DrawerState; onClose: () => void; onSaved: () => void }) {
...
export function RoomsClient({ initial }: { initial: RoomsOverview }) {
  ...
  const handleAssignWaiting = async () => {
    ...
    try {
      const result = await assignWaitingPlayers();
      if (!result.ok) {
        setAssigning(false);
  ...
  return (
    <main className="rc-admin-page">
      ...
      <RoomDrawer state={drawer} onClose={() => setDrawer(null)} onSaved={refresh} />
    </main>
  );
}
