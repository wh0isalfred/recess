...
export function OverviewView({ slug, data }: { slug: string; data: EventOverview }) {
  ...
  return (
    <main className="rc-admin-page">
      ...
      <section className="rc-admin-card">
        ...
        {event.status === "DRAFT" ? (
          <LifecycleButton slug={slug} action="registration" disabled={false} />
        ) : (
          <LifecycleButton
            slug={slug}
            action="checkin"
            disabled={event.status !== "REGISTRATION" && event.status !== "REGISTRATION_CLOSED"}
          />
        )}
      </section>
      ...
      <section className="rc-admin-card">
        <div className="rc-admin-card-head">
          ...
          <a href={`/admin/events/${slug}/rooms`} className="rc-admin-view-all">VIEW ALL →</a>
        </div>
